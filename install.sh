...
show_dns_configuration_guide() {
    ...
    echo "  • Wildcard A record (*.$DOMAIN) allows HTTP/HTTPS access to subdomains"
    echo "  ${RED}• NS record is MANDATORY for DNS query logging (not optional!)${NC}"
    echo "  ${RED}• Without NS delegation, DNS queries will NOT be logged${NC}"
    echo "  • A records enable HTTP access, NS records enable DNS logging"
    echo ""
    echo -e "${CYAN}What each record does:${NC}"
    echo "  A $DOMAIN          → HTTP access to main domain"
    echo "  A *.$DOMAIN        → HTTP access to callback subdomains"
    echo "  ${GREEN}NS $DOMAIN         → Delegates DNS queries to your server${NC}"
    echo "  ${GREEN}A ns1.$DOMAIN      → IP of your DNS server${NC}"
    echo ""
    echo -e "${CYAN}Verify DNS propagation:${NC}"
    echo "  dig $DOMAIN @8.8.8.8"
    echo "  dig test.$DOMAIN @8.8.8.8"
    echo "  ${GREEN}dig NS $DOMAIN @8.8.8.8${NC}  ← Verify NS delegation"
    echo "  Online: https://dnschecker.org"
    echo ""
    echo -e "${RED}⚠ WARNING: Without NS records, DNS queries will NOT appear in logs!${NC}"
    echo -e "${YELLOW}See NS-RECORDS-SETUP-GUIDE.md for detailed instructions${NC}"
    echo ""
}
...
setup_ssl() {
    ...
    if [ "$USE_SSL" = true ]; then
        ...
        if [ -d "/etc/letsencrypt/live/$DOMAIN" ]; then
            ...
            echo -e "${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
            echo ""
            echo -e "${GREEN}Available Options (All support ${GREEN}DNS Logging${NC}${GREEN} + ${GREEN}Wildcard${NC}${GREEN}):${NC}"
            echo ""
            echo "  ${CYAN}1. Let's Encrypt DNS Challenge${NC} (${GREEN}RECOMMENDED${NC})"
            echo "     • Wildcard certificate support: *.${DOMAIN}"
            echo "     • Manual TXT record verification"
            echo "     • No rate limit issues (single wildcard cert for all subdomains)"
            echo "     • Command: certbot --manual --preferred-challenges dns"
            echo ""
            echo "  ${CYAN}2. acme.sh with Cloudflare DNS API${NC} (${GREEN}Fully Automatic${NC})"
            echo "     • Automatic wildcard certificate"
            echo "     • Auto-renewal via Cloudflare API"
            echo "     • Requires Cloudflare API token"
            echo "     • ${GREEN}DNS logging works${NC} (uses DNS-only mode, not proxy)"
            echo ""
            echo "  ${YELLOW}3. Skip SSL setup${NC} (HTTP only - not recommended)"
            echo ""
            echo -e "${YELLOW}Why only DNS Challenge?${NC}"
            echo "  ✓ Supports wildcard certificates (*.alewo.xyz)"
            echo "  ✓ Single certificate covers ALL subdomains"
            echo "  ✓ No rate limit issues (5 certs/week becomes irrelevant)"
            echo "  ✓ Works with DNS logging perfectly"
            echo "  ✗ Requires manual TXT record setup (option 1) or API (option 2)"
            echo ""
            ...
            case $SSL_OPTION in
                1)
                    # Let's Encrypt DNS Challenge (RECOMMENDED)
                    setup_letsencrypt_dns_challenge
                    ;;
                2)
                    # acme.sh with Cloudflare DNS API
                    setup_acme_sh
                    ;;
                3)
                    # Skip SSL
                    ...
                    ;;
                *)
                    log_warning "Invalid option, using Let's Encrypt DNS Challenge (Option 1)..."
                    setup_letsencrypt_dns_challenge
                    ;;
            esac
        fi
...
# Self-signed SSL (used as fallback when certificate issuance fails)
setup_self_signed_ssl() {
...
configure_nginx() {
    ...
    if [ "$USE_SSL" = true ]; then
        ...
        # Priority: acme.sh (Cloudflare DNS API) > Let's Encrypt > Self-signed fallback
        if [ -f "/etc/nginx/ssl/$DOMAIN.cert" ]; then
            # Use acme.sh certificate (from Cloudflare DNS API setup)
            SSL_CONFIG="
...
# Interactive NS Delegation Tutorial
show_ns_delegation_tutorial() {
    # Skip if using IP instead of domain
    ...
    echo ""
    echo -e "${PURPLE}════════════════════════════════════════════════════════════${NC}"
    echo -e "${PURPLE}       DNS DELEGATION SETUP - MANDATORY FOR DNS LOGGING      ${NC}"
    echo -e "${PURPLE}════════════════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "${YELLOW}⚠️  IMPORTANT: Without NS delegation, DNS queries will NOT be logged!${NC}"
    echo -e "${YELLOW}⚠️  Your AlewoCallback will only log HTTP requests (not DNS).${NC}"
    echo ""
    echo -e "${CYAN}Do you want to see the step-by-step NS delegation tutorial?${NC}"
    echo -e "  ${GREEN}y${NC} - Yes, show me the tutorial"
    echo -e "  ${GREEN}n${NC} - No, I'll configure it later"
    echo ""
    read -p "Your choice [y/n] (default: y): " SHOW_TUTORIAL
    SHOW_TUTORIAL=${SHOW_TUTORIAL:-y}

    if [ "$SHOW_TUTORIAL" != "y" ]; then
        log_info "Skipping NS delegation tutorial"
        echo -e "${YELLOW}Note: You can find the complete guide in:${NC}"
        echo -e "  ${CYAN}docs/NS-RECORDS-SETUP-GUIDE.md${NC}"
        echo -e "  ${CYAN}docs/TUTORIAL-FULL-DELEGATION-ALEWO-XYZ.md${NC}"
        echo ""
        return 0
    fi
    ...
    echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║                                                               ║${NC}"
    echo -e "${GREEN}║       NS DELEGATION TUTORIAL FOR ${DOMAIN}${NC}"
    echo -e "${GREEN}║                                                               ║${NC}"
    echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${CYAN}Why NS Delegation is MANDATORY:${NC}"
    echo ""
    echo -e "Without NS delegation:"
    echo -e "  ❌ DNS queries go to your domain registrar's DNS"
    echo -e "  ❌ AlewoCallback DNS server is NEVER queried"
    echo -e "  ❌ DNS logs will NOT appear (HTTP logs will still work)"
    echo ""
    echo -e "With NS delegation:"
    echo -e "  ✅ DNS queries come directly to AlewoCallback"
    echo -e "  ✅ All DNS queries are logged"
    echo -e "  ✅ Works exactly like Burp Collaborator!"
    echo ""
    read -p "Press Enter to continue..."

    clear
    echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║  STEP 1: CREATE CHILD NAMESERVERS (GLUE RECORDS)             ║${NC}"
    echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${CYAN}Go to your domain registrar control panel${NC}"
    echo ""
    echo -e "Navigate to: ${YELLOW}DNS / Nameservers → Child nameservers${NC}"
    echo ""
    echo -e "${CYAN}Add the following child nameservers:${NC}"
    echo ""
    echo -e "  ${GREEN}Nameserver 1:${NC}"
    echo -e "    Hostname: ${YELLOW}ns1.${DOMAIN}${NC}"
    echo -e "    IP Address: ${YELLOW}${PUBLIC_IP}${NC}"
    echo ""
    echo -e "  ${GREEN}Nameserver 2 (optional for redundancy):${NC}"
    echo -e "    Hostname: ${YELLOW}ns2.${DOMAIN}${NC}"
    echo -e "    IP Address: ${YELLOW}${PUBLIC_IP}${NC}"
    echo ""
    echo -e "${CYAN}Note:${NC} Both use the same IP because you have one server."
    echo -e "${CYAN}Note:${NC} Some registrars call this 'Glue Records' or 'Host Records'."
    echo ""
    read -p "Press Enter after you've created the child nameservers..."

    clear
    echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║  STEP 2: CHANGE DOMAIN NAMESERVERS                            ║${NC}"
    echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${CYAN}Go back to: DNS / Nameservers${NC}"
    echo ""
    echo -e "${CYAN}Select: ${YELLOW}Use custom nameservers${NC}"
    echo ""
    echo -e "${CYAN}Replace the current nameservers with:${NC}"
    echo ""
    echo -e "  ${GREEN}Nameserver 1:${NC} ${YELLOW}ns1.${DOMAIN}${NC}"
    echo -e "  ${GREEN}Nameserver 2:${NC} ${YELLOW}ns2.${DOMAIN}${NC}"
    echo ""
    echo -e "${YELLOW}⚠️  IMPORTANT:${NC}"
    echo -e "  - You MUST create child nameservers (Step 1) FIRST"
    echo -e "  - Then change nameservers (Step 2)"
    echo -e "  - Both steps are REQUIRED!"
    echo ""
    echo -e "${CYAN}Your registrar will show a warning:${NC}"
    echo -e '  "Changing nameservers will affect how your domain is managed."'
    echo ""
    echo -e "${GREEN}Click 'Confirm' or 'Save' to proceed.${NC}"
    echo ""
    read -p "Press Enter after you've changed the nameservers..."

    clear
    echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║  STEP 3: WAIT FOR DNS PROPAGATION                             ║${NC}"
    echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${CYAN}DNS propagation time:${NC}"
    echo -e "  ⏱️  Minimum: ${YELLOW}1-2 hours${NC}"
    echo -e "  ⏱️  Average: ${YELLOW}4-6 hours${NC}"
    echo -e "  ⏱️  Maximum: ${YELLOW}24-48 hours${NC}"
    echo ""
    echo -e "${CYAN}What to do while waiting:${NC}"
    echo -e "  ☕ Grab some coffee"
    echo -e "  📺 Watch a movie"
    echo -e "  🎮 Play some games"
    echo -e "  💤 Get some rest"
    echo ""
    echo -e "${YELLOW}Set a reminder to check back in 2-4 hours!${NC}"
    echo ""
    read -p "Press Enter to continue..."

    clear
    echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║  STEP 4: VERIFY NS DELEGATION                                 ║${NC}"
    echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${CYAN}After 2-4 hours, verify the nameserver change:${NC}"
    echo ""
    echo -e "${YELLOW}Command:${NC}"
    echo -e "  ${GREEN}dig NS ${DOMAIN} @8.8.8.8${NC}"
    echo ""
    echo -e "${CYAN}Expected output (SUCCESS):${NC}"
    echo -e "  ;; ANSWER SECTION:"
    echo -e "  ${DOMAIN}.      3600    IN      NS      ${YELLOW}ns1.${DOMAIN}.${NC}"
    echo -e "  ${DOMAIN}.      3600    IN      NS      ${YELLOW}ns2.${DOMAIN}.${NC}"
    echo ""
    echo -e "${GREEN}✅ If you see this, nameservers have propagated!${NC}"
    echo ""
    echo -e "${YELLOW}Verify nameserver IP:${NC}"
    echo -e "  ${GREEN}dig ns1.${DOMAIN} @8.8.8.8${NC}"
    echo ""
    echo -e "${CYAN}Expected output:${NC}"
    echo -e "  ;; ANSWER SECTION:"
    echo -e "  ns1.${DOMAIN}.  3600    IN      A       ${YELLOW}${PUBLIC_IP}${NC}"
    echo ""
    echo -e "${GREEN}✅ If you see this, glue records are working!${NC}"
    echo ""
    read -p "Press Enter to continue..."

    clear
    echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║  STEP 5: TEST DNS LOGGING                                     ║${NC}"
    echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${CYAN}Test if DNS queries reach AlewoCallback:${NC}"
    echo ""
    echo -e "${YELLOW}1. Run this command on your computer:${NC}"
    echo -e "  ${GREEN}dig test123.${DOMAIN} @8.8.8.8${NC}"
    echo ""
    echo -e "${CYAN}Expected output:${NC}"
    echo -e "  ;; ANSWER SECTION:"
    echo -e "  test123.${DOMAIN}.  300    IN      A       ${YELLOW}${PUBLIC_IP}${NC}"
    echo ""
    echo -e "${YELLOW}2. Check logs on your server:${NC}"
    echo -e "  ${GREEN}sudo alewo-callback logs | tail -20${NC}"
    echo ""
    echo -e "${CYAN}Expected in logs:${NC}"
    echo -e "  ${GREEN}DNS Query: test123.${DOMAIN} (A) from 8.8.8.8${NC}"
    echo -e "  ${GREEN}✓ DNS query logged for subdomain: test123${NC}"
    echo ""
    echo -e "${YELLOW}3. Check AlewoCallback Dashboard:${NC}"
    echo -e "  - Login to ${PROTOCOL}://${DOMAIN}"
    echo -e "  - Create subdomain: ${YELLOW}test123${NC}"
    echo -e "  - Run: ${GREEN}dig test123.${DOMAIN} @8.8.8.8${NC}"
    echo -e "  - Refresh dashboard"
    echo -e "  - ${GREEN}✅ DNS query should appear!${NC}"
    echo ""
    read -p "Press Enter to continue..."

    clear
    echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║  FINAL RESULT: HOW IT WORKS                                   ║${NC}"
    echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${CYAN}After NS delegation is complete:${NC}"
    echo ""
    echo -e "${YELLOW}User queries: abc123.${DOMAIN}${NC}"
    echo -e "    ↓"
    echo -e "  Public DNS (8.8.8.8): 'What's the nameserver for ${DOMAIN}?'"
    echo -e "    ↓"
    echo -e "  Registry: 'ns1.${DOMAIN} (IP: ${PUBLIC_IP})'"
    echo -e "    ↓"
    echo -e "  Query sent DIRECTLY to ${YELLOW}${PUBLIC_IP}:53${NC} (AlewoCallback DNS)"
    echo -e "    ↓"
    echo -e "  ${GREEN}✅ DNS query LOGGED in database${NC}"
    echo -e "  ${GREEN}✅ Returns IP: ${PUBLIC_IP}${NC}"
    echo -e "    ↓"
    echo -e "  User's browser connects to ${PUBLIC_IP}"
    echo -e "    ↓"
    echo -e "  ${GREEN}✅ HTTP request LOGGED in database${NC}"
    echo ""
    echo -e "${CYAN}Dashboard shows BOTH:${NC}"
    echo -e "  ${GREEN}✅ DNS query log${NC}  (Type: DNS, Query: abc123.${DOMAIN})"
    echo -e "  ${GREEN}✅ HTTP request log${NC} (Method: GET, Path: /)"
    echo ""
    echo -e "${GREEN}🎉 EXACTLY LIKE BURP COLLABORATOR! 🎉${NC}"
    echo ""
    echo ""
    echo -e "${YELLOW}📚 Complete documentation available at:${NC}"
    echo -e "  ${CYAN}docs/NS-RECORDS-SETUP-GUIDE.md${NC} - Comprehensive NS setup guide"
    echo -e "  ${CYAN}docs/DNS-TESTING-GUIDE.md${NC} - DNS testing instructions"
    echo ""
...
show_completion() {
    ...
    if [ "$USE_SSL" = true ] && [ -d "/etc/letsencrypt/live/$DOMAIN" ]; then
        ...
        echo -e "${YELLOW}Note: To enable SSL later with wildcard support, run:${NC}"
        echo -e "  sudo certbot certonly --manual --preferred-challenges dns \\"
...
EOFCONFIG
    ...
    # Show NS delegation tutorial (interactive)
    show_ns_delegation_tutorial
